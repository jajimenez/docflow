...
def get_incoming_pdf_file_paths() -> list[str]:
    ...
    pdf_paths = list(Path(settings.pdf_pending_dir).glob("*.pdf"))
    logger.info(PENDING_PDF_FILES_FOUND.format(len(pdf_paths)))
...
def save_document_batch(db_url: str, pdf_file_paths: list[str]) -> list[str]:
    ...
    with get_session(db_url) as session:
        for path in pdf_file_paths:
            ...
            doc = get_document(session, file_path=path)
...
def process_document_batch(db_url: str, doc_ids: list[str]):
    ...
    with get_session(db_url) as session:
        for i in doc_ids:
            ...
            try:
                ...
                pending_path = Path(doc.file_path)
                ...
                destination_path = Path(settings.pdf_processed_dir) / pending_name
                ...
                if pending_name is not None:
                    destination_path = Path(settings.pdf_failed_dir) / pending_name
            finally:
